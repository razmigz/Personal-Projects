...
#data from 2015; use that as current year
...
predictors.df <- df %>% 
  dplyr::select(-c(price, zipcode, waterfront, view))
...
pre_proc_val <- preProcess(train[,2:20], method = c("center", "scale"))

train[, 2:20] = predict(pre_proc_val, train[,2:20])
test[, 2:20] = predict(pre_proc_val, test[,2:20])
